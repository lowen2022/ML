...
class Translator(object):
    ...
    def translate_batch(self, batch, data, fast=False):
        ...
        Args:
           ...
           #so here are the reason for why can't we get top5 result?
    ...
    def _fast_translate_batch(self,
        ...
        print("beam_size367",beam_size)#5
        print("fast",self.fast)#True
        ...
        print("beam_size384", beam_size)  # 5
        ...
        i=0
        for step in range(max_length):#max_length=200
            # i=i+1

            # print(i)# need to check thr process
            ...
            # select_indices=torch.tensor(select_indices, dtype=torch.int64)
            alive_seq = torch.cat(
            ...
            if return_attention:
                ...
                current_attn = attn["std"].index_select(1, select_indices.long())#.long() for int64
                if alive_attn is None:
                    alive_attn = current_attn
                else:
                    alive_attn = alive_attn.index_select(1, select_indices.long())#long
            ...
            if type(memory_bank) == tuple:
                ...
                memory_bank = memory_bank.index_select(1, select_indices.long())#long()
            memory_lengths = memory_lengths.index_select(0, select_indices.long())#long
            dec_states.map_batch_fn(
                lambda state, dim: state.index_select(dim, select_indices.long()))#long
